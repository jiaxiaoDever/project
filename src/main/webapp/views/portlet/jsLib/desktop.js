...
myLib.desktop={
	//常量配置
	CONSTS : {//需要扩展改造
			CLASS: [ 'themes/default/css/desktop.css',
						...
						paperbgimg:"themes/default/images/blue_glow.jpg",
	},
	portlets : [],
...
myLib.desktop.win={
    ...
    newWin:function(options){
		...
		if(options.WindowAnimation=='none'){
		 ...
		 $newWin
		 .css({"left":$iconOffset !=null ?$iconOffset.left:0,"top":$iconOffset != null ? $iconOffset.top:0,"z-index":zindex})
		 .addClass("loading")
		 .show()
		 .animate({ 
	...
	contextMenu:function(data){
			...
			var icon_right_button=[
			    	   				[{
			    						text:"删除此portlet",
			    						func:function(){
			    							removeUserPortlet(data.id);
			    						} 
			    					}]
			    					];
			myLib.desktop.contextMenu($("#"+data.id),icon_right_button,"icon_menu"+data.id,12);
	},
	removeIcon:function(id){
		//移除win
		var winId = "myWin_"+id;
		var win = $("#"+winId);
		myLib.desktop.win.closeWin(win);
		//移除menu
		var menuId = "icon_menu"+id;
		$("#"+menuId).remove();
		//移除icon
		$("#"+id).remove();
