...
%>
<script type="text/javascript" src="<%=basePath %>/static/yhui/debug/js/base/jquery-1.8.3.js"></script>
	...
	<script>
		...
		function changeJobStatus(jobId, cmd) {
			...
			$.ajax({
				...
				dataType : "JSON",
				cache : false,
				url : "${basePath}task/changeJobStatus.app",
				data : {
					jobId : jobId,
					cmd : cmd
				},
		...
		function updateCron(jobId) {
			...
			if (cron) {
				...
				$.ajax({
					...
					dataType : "JSON",
					cache : false,
					url : "${basePath}task/updateCron.app",
					data : {
						jobId : jobId,
						cron : cron
					},
