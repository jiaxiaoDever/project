...
* http://jqueryui.com
* yhui: remove the datepicker, make accordion|autocomplete|tabs|tooltip|dialog small changes
...
(function() {
	...
	$.ui.le7 = parseFloat( uaMatch[ 1 ], 10 ) <= 7;
})();
...
$.widget( "ui.autocomplete", {
			...
			menufocus: function( event, ui ) {
				...
				if ( false !== this._trigger( "focus", event, { item: item } ) ) {
					...
					if ( event.originalEvent && /^key/.test( event.originalEvent.type ) ) {
						this._value( item[ key ? key : "value"] );
					}
				} else {
	...
	_suggest: function( items ) {
        ...
        // yhui: must position first, and then show
		//ul.show();
		this._resizeMenu();
		ul.css( { top:0, left:0 } ).position( $.extend({
			of: this.element
		}, this.options.position ) ).show();
	...
	_renderItem: function( ul, item ) {
		...
		return $( "<li>" )
			.append( $( "<a>" ).text( item[ key ? key : "label" ] ) )
			.appendTo( ul );
	},
...
$.widget("ui.dialog", {
	...
	_makeDraggable: function() {
		...
		// yhui: add a model that have dragHelper
		if ( !options.dragHelper ) {
			...
			this.uiDialog.draggable({
				...
				helper: function(){
					...
					return "<div style = 'border: 1px dashed #000; background: #DDD; filter:alpha(opacity=60); opacity: 0.6; width:" 
								+ width + "px; height:" + height + "px; z-index: 2001;'></div>";
				},
				stop: function( event, ui ) {
					options.position = [
						ui.position.left - that.document.scrollLeft(),
...
$.fn.position = function( options ) {
	...
	return this.each(function() {
		var collisionPosition, using,
			...
			// just don't know this variable
			collisionWidth = elemWidth + marginLeft + parseCss( this, "marginRight" ) + scrollInfo.width,
			collisionHeight = elemHeight + marginTop + parseCss( this, "marginBottom" ) + scrollInfo.height,
			position = $.extend( {}, basePosition ),	// copy
...
$.widget( "ui.tabs", {
	...
	_setupHeightStyle: function( heightStyle ) {
		...
		if ( heightStyle === "fill" ) {
			...
			//yhui: just forget the border width of the element
			var borderWidth = parseInt( this.element.css( "borderBottomWidth" ) ) * 2;
...
$.widget( "ui.tooltip", {
	...
	_open: function( event, target, content ) {
		...
		// yhui: update the position when update the content
		if ( tooltip.length ) {
			positionOption.of = target;
        ...
        // yhui: hide first and then to position it
		tooltip.hide().find( ".ui-tooltip-content" ).html( content );
...
/*!
 * jQuery UI Selectmenu 1.10.1pre
 * http://jqueryui.com
 *
